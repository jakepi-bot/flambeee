...
- Cinder cross-day streak history across the daily reset (the Story 040 note: `checkDailyReset()` rebuilds the day record, so the streak line normally reads "broken" on a real return; preserving history is a separate decision)
